class Product < ActiveRecord::Base
  ...
  scope :random, ->(myfield, mn=0.0001, mx=1000, selectedids=1) { select("#{myfield} as test_condition, *").where("#{myfield} >= #{mn} and #{myfield} <= #{mx} and id <> #{selectedids}").offset(rand(Product.totalavailable(myfield, mn, mx))).limit(1) }
end
